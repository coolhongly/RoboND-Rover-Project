...
# Identify pixels of rocks
def color_thresh_rocks(img, rgb_thresh=(100, 100, 50)):
    # Create an array of zeros same xy size as img, but single channel
    color_select = np.zeros_like(img[:,:,0])
    # Require that each pixel be above all three threshold values in RGB
    # above_thresh will now contain a boolean array with "True"
...
def perception_step(Rover):
    ...
    obstacle = color_thresh_obstacles(warped)
    rock = color_thresh_rocks(warped)
    ...
    nav_dists, nav_angles = to_polar_coords(navigable_x_rover, navigable_y_rover)
    rock_dists, rock_angles = to_polar_coords(rock_x_rover, rock_y_rover)
    Rover.nav_dists = nav_dists
    if len(rock_angles) > 0 and np.mean(rock_dists) < 100:
        Rover.find_sample = True
        Rover.nav_angles = rock_angles
        Rover.max_vel = Rover.sample_max_vel
    else:
        Rover.find_sample = False
        Rover.nav_angles = nav_angles
        Rover.max_vel = Rover.normal_max_vel
